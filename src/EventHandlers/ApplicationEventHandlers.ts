...
export async function ddChangeInfo_StaffDirectoryItems_OnChangeManagerInfo(sender: any, args: IEventArgs): Promise<void> {
    const logic = new ApplicationLogic();
    await logic.onBusinessTripCommanderChanged(sender, sender.layout);
}

export async function ddChangeInfo_Number_OnChangeSumBusinessTrip(sender: any, args: IEventArgs): Promise<void> {
    const logic = new ApplicationLogic();
